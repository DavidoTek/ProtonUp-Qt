...
# TinkerGame
# Copyright (C) 2026 TinkerGame contributors, based on the SteamTinkerLaunch ctmod by DavidoTek
...
<br/><br/>
TinkerGame is a community fork of SteamTinkerLaunch.
...
class CtInstaller(QObject):
    ...
    def __config_change_language(self, tg_cfg_path: str, lang_file: str) -> bool:
        """
        Change the language in the TinkerGame global.conf configuration
        Example: __config_change_language('~/.config/tinkergame', 'german.txt')
        Return Type: bool
        """
        tg_global_conf = os.path.join(os.path.expanduser(tg_cfg_path), 'global.conf')
        new_lang = lang_file.replace('.txt', '')

        if not os.path.isfile(tg_global_conf):
            return False

        with open(tg_global_conf, 'r+') as f:
            c = f.readlines()
            f.seek(0)
            for line in c:
    ...
    def get_tool(self, version, install_dir, temp_dir):
        ...
        with tarfile.open(destination, "r:gz") as tar:
            ...
            if os.path.exists(constants.STEAM_TG_INSTALL_PATH) and len(os.listdir(constants.STEAM_TG_INSTALL_PATH)) > 0:
            ...
            if not os.path.exists(constants.STEAM_TG_INSTALL_PATH):
                os.mkdir(constants.STEAM_TG_INSTALL_PATH)
            os.chdir(constants.STEAM_TG_INSTALL_PATH)

            tar.extractall(constants.STEAM_TG_INSTALL_PATH)

            tarname = tar.getnames()[0]
            
            # Location of TinkerGame script to add to path later
            old_tg_path = os.path.join(constants.STEAM_TG_INSTALL_PATH, tarname)
            tg_path = os.path.join(constants.STEAM_TG_INSTALL_PATH, 'prefix')

            # Rename folder ~/tg/<tarname> to ~/tg/prefix
            os.rename(old_tg_path, tg_path)
            os.chdir(tg_path)

            # ProtonUp-Qt Flatpak: Run TinkerGame on host system
            tg_proc_prefix = ['flatpak-spawn', '--host'] if constants.IS_FLATPAK else []

            # If on Steam Deck, run script for initial Steam Deck config
            # On Steam Deck, TinkerGame is installed to "/home/deck/tg/prefix"
            ...
            if "steamos" in self.distinfo:
                ...
                subprocess.run(tg_proc_prefix + ['./tinkergame'])
                ...
                tg_lang = tg_langs[syslang] if syslang in tg_langs else tg_langs['en_US']
                tg_lang_path = os.path.join(constants.STEAM_TG_CONFIG_PATH, 'lang')

                # Generate config file structure and copy relevant lang file
                os.makedirs(tg_lang_path, exist_ok=True)
                if not os.path.isfile(os.path.join(tg_lang_path, 'english.txt')):
                    shutil.copyfile('lang/english.txt', os.path.join(tg_lang_path, 'english.txt'))
                if not os.path.isfile(os.path.join(tg_lang_path, tg_lang)):
                    shutil.copyfile(f'lang/{tg_lang}', os.path.join(tg_lang_path, tg_lang))
                subprocess.run(tg_proc_prefix + ['./tinkergame', f'lang={tg_lang.replace(".txt", "")}'])
                self.__config_change_language(constants.STEAM_TG_CONFIG_PATH, tg_lang)
            ...
            if should_add_path:
                ...
                pup_tg_path_date = f'# Added by ProtonUp-Qt on {datetime.datetime.now().strftime("%d-%m-%Y %H:%M:%S")}'
                pup_tg_path_line = f'if [ -d "{tg_path}" ]; then export PATH="$PATH:{tg_path}"; fi'
                present_shell_files = [
                    os.path.join(constants.HOME_DIR, f) for f in os.listdir(constants.HOME_DIR) if os.path.isfile(os.path.join(constants.HOME_DIR, f)) and f in constants.STEAM_TG_SHELL_FILES
                ]
                if os.path.exists(constants.STEAM_TG_FISH_VARIABLES):
                    present_shell_files.append(constants.STEAM_TG_FISH_VARIABLES)

                for shell_file in present_shell_files:
                    with open(shell_file, 'r+') as mfile:
                        tg_already_in_path = constants.STEAM_TG_INSTALL_PATH in [line for line in mfile.readlines()]
                        if not tg_already_in_path:
                            # Add Fish user path, preserving any existing paths 
                            if 'fish' in mfile.name:
                                mfile.seek(0)
                                curr_fish_user_paths = get_fish_user_paths(mfile)
                                curr_fish_user_paths.insert(0, tg_path)
                                updated_fish_user_paths = '\\x1e'.join(curr_fish_user_paths)
                                pup_tg_path_line = f'SETUVAR fish_user_paths:{updated_fish_user_paths}'
            ...
            subprocess.run(tg_proc_prefix + ['./tinkergame', 'compat', 'add'])
        ...
        if os.path.exists(protondir):
            ...
            tg_filename = 'tinkergame'
            tg_ver = ''
            with open(os.path.join(protondir, tg_filename)) as tg_script:
                for i, line in enumerate(tg_script):
                    if 'PROGVERS' in line:
                        tg_ver = line.split('=')[1].replace('"', '')  # E.g. turn `PROGVERS="v14.0.20260801"` into `v14.0.20260801`
                        
                        print(f'Storing TinkerGame version from TinkerGame script file as {tg_ver}')
